...
ifeq ($(MODE), cabal)
> @install -m 0755 \
>   "$(shell cabal list-bin $(CABAL_ARGS) bm)" \
>   "$(bindir)/bm"
else
...
install-doc: # install documentation to PREFIX/share/doc/bm-haskell
> @mkdir -p "$(docdir)"
...
> @mkdir -p "$(man1dir)"
> @install -m 0644 -T <(gzip -c doc/bm.1) "$(man1dir)/bm.1.gz"
.PHONY: install-man
...
> @pandoc -s -t man -o doc/bm.1 \
>   --variable header="bm Manual" \
